...
/**
 * Tells the client what requests the server will accept. Also returns whether
 * the server supports sending patches, which is not really anything to do with
 * requests!
 * 
 * @author Robert Greig
 */
...
class ValidRequestsResponse implements Response {
    ...
    public void process(final LoggedDataInputStream dis, final ResponseServices services) throws ResponseException {
        try {
            final String validRequests = dis.readLine();
