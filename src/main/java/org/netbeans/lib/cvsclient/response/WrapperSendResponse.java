...
public class WrapperSendResponse implements Response {
    ...
    public static Map<StringPattern, KeywordSubstitutionOptions> parseWrappers(final String line) {
        ...
        Map<StringPattern, KeywordSubstitutionOptions> wrappersMap = null;
            ...
            if (option.equals("-k")) { // NOI18N
                ...
                if (wrappersMap == null) {
                    if (!tokenizer.hasMoreTokens()) {
                        wrappersMap = Collections.<StringPattern, KeywordSubstitutionOptions> singletonMap(pattern,
                                        keywordOption);
                    } else {
                        wrappersMap = new LinkedHashMap<StringPattern, KeywordSubstitutionOptions>();
    ...
    public void process(final LoggedDataInputStream dis, final ResponseServices services) throws ResponseException {
        try {
            ...
            final String wrapperSettings = dis.readLine();
            final Map<StringPattern, KeywordSubstitutionOptions> wrappers = parseWrappers(wrapperSettings);
