...
class UpdatedResponse implements Response {
     ...
     * 
     */
    private static final long serialVersionUID = 4621842657150978788L;
    ...
    /**
     * fullpath to the file being processed. Is used in Merged response when
     * sending EnhancedMessageEvent
     */
    ...
    public void process(final LoggedDataInputStream dis, final ResponseServices services) throws ResponseException {
        try {
            ...
            final String nextLine = dis.readLine();

            final boolean useGzip = (nextLine.charAt(0) == 'z');

            final int length = Integer.parseInt(useGzip ? nextLine.substring(1) : nextLine);
            ...
            if (this instanceof CreatedResponse) {
                if (newFile.exists()) {
                    skip(dis, length);
                    // Fire "C file.txt" type of event. This event is caught
                    // directly by clients of the library
                    final DefaultFileInfoContainer fic = new DefaultFileInfoContainer();
                    fic.setType("C");
                    fic.setFile(newFile);
            ...
            final FileHandler fileHandler = useGzip ? services.getGzipFileHandler() : services
            ...
            if (newFile.exists()) {
                final FileAddedEvent e = new FileAddedEvent(this, filePath);
                services.getEventManager().fireCVSEvent(e);
            } else {
                final FileUpdatedEvent e = new FileUpdatedEvent(this, filePath);
                services.getEventManager().fireCVSEvent(e);
            }
    ...
    private void skip(final LoggedDataInputStream dis, int length) throws IOException {
        while (length > 0) {
    ...
    /**
     * Returns the Conflict field for the file's entry. Can be overriden by
     * subclasses. (For example the MergedResponse that sets the
     ...
     */
    protected String getEntryConflict(final Date date, final boolean hadConflicts) {
