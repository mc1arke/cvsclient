 ...
 * 
 * <p>
 * When the method is not defined, we treat it as local or ext method depending
 * on whether the hostname is present or not. This gives us two different
 * formats:
 * </p>
 * 
 * <h4>1. Local format</h4> <code>[:method:]/path/to/repository</code> or
 ...
 * </li> gserver and kserver are not included. Environment variables are not
 * used (like CVS_RSH).
 * </p>
 * <p>
...
public class CVSRoot {
    ...
    // port (default = 0) 0 means that port is not used and the protocol will
    ...
    /**
     * Parse the CVSROOT string into CVSRoot object. The CVSROOT string must be
     * of the form
     * [:method:][[user][:password]@][hostname:[port]]/path/to/repository
     */
    public static CVSRoot parse(final String cvsroot) throws IllegalArgumentException {
        return new CVSRoot(cvsroot);
    }

    /**
     * Construct CVSRoot from Properties object. The names are exactly the same
     * as the attribute names in this class.
     */
    public static CVSRoot parse(final Properties props) throws IllegalArgumentException {
        return new CVSRoot(props);
    }

    /**
     * This constructor allows to construct CVSRoot from Properties object. The
     * names are exactly the same as the attribute names in this class.
     */
    protected CVSRoot(final Properties props) throws IllegalArgumentException {

        final String mtd = props.getProperty("method");
        if (mtd != null) {
            method = mtd.intern();
        }
        ...
        if (hostname.length() == 0) {
            hostname = null;
            // this.localFormat = this.hostname == null ||
        ...
        if (hostname != null) {
            ...
            try {
                final int p = Integer.parseInt(props.getProperty("port"));
                if (p > 0) {
                    port = p;
                } else {
        ...
        final String r = props.getProperty("repository");
        if (r == null) {
     ...
     * 
     * For CVSNT compatability it also supports following local repository path
     * format
     * 
     * driveletter:path\\path\\path
     * 
     */
    protected CVSRoot(final String cvsroot) throws IllegalArgumentException {
        ...
        if (cvsroot.startsWith(":") == false) {
            ...
            if (localFormat == false) {
                ...
                if (colonPosition < 0) {
                    // No colon => server format, but there must be a '/' in the
                    // middle
                    final int slash = cvsroot.indexOf('/');
                    if (slash < 0) {
            ...
            final int semicolonPosition = cvsroot.indexOf(";", 1);

            if ((semicolonPosition != -1) && (semicolonPosition < colonPosition)) {
                // method has options
                methodNameEnd = semicolonPosition;
                final String options = cvsroot.substring(semicolonPosition + 1, colonPosition);
            ...
            method = cvsroot.substring(1, methodNameEnd).intern();
            ...
            localFormat = isLocalMethod(method);
        }
        ...
        if (localFormat) {
             ...
             * ALGORITHM: - find the first '@' character - not found: - find the
             * first ':' - not found - find the first '/' - not found -
             * exception - found - parse hostname/path/to/repository - found -
             * parse rest - found - find the following ':' character - not found
             * - exception - found parse rest
             */
            ...
            int pathBegin = -1;
            final int hostColon = cvsroot.indexOf(':', startSearch);
            if (hostColon == -1) {
            ...
            final int at = userPasswdHost.lastIndexOf('@');
            if (at == -1) {
                // there is no user or password, only hostname before port
                if (userPasswdHost.length() > 0) {
                    hostname = userPasswdHost;
                }
            } else {
                // there is user, password or both before hostname
                // up = username, password or both
                final String up = userPasswdHost.substring(0, at);
                if (up.length() > 0) {
                    final int upDivider = up.indexOf(':');
                    if (upDivider != -1) {
                        username = up.substring(0, upDivider);
                        password = up.substring(upDivider + 1);
                    } else {
                        username = up;
                    }
                }
                ...
                // hostname
                hostname = userPasswdHost.substring(at + 1);
            }
            ...
            /*
             * Now we are left with port (optional) and repository after
             * hostColon pr = possible port and repository
             */
            if (hostColon > 0) {
     ...
     * connection. The connection is local if and only if the host name is
     ...
     * Password is never included in cvsroot string representation. Use
     * getPassword to get it.</li>
     * </ul>
     */
     ...
     * <li>1 = refers to same repository on the same machine using same method
     * on same port and same user</li>
     ...
     * </ul>
     */
    public int getCompatibilityLevel(final CVSRoot compared) {

        if (equals(compared)) {
            return 0;
        }

        final boolean sameRepository = isSameRepository(compared);
        final boolean sameHost = isSameHost(compared);
        final boolean sameMethod = isSameMethod(compared);
        final boolean samePort = isSamePort(compared);
        final boolean sameUser = isSameUser(compared);
    ...
    private boolean isSameRepository(final CVSRoot compared) {
        if (repository.equals(compared.repository)) {
            return true;
        }
        try {
            if ((new File(repository)).getCanonicalFile().equals(new File(compared.repository).getCanonicalFile())) {
                return true;
            } else {
                return false;
            }
        } catch (final IOException ioe) {
            // something went wrong when invoking getCanonicalFile() so return
            // false
            return false;
        }
    }

    private boolean isSameHost(final CVSRoot compared) {
        final String comparedHostName = compared.getHostName();
        if (hostname == comparedHostName) {
            return true;
        }
        if (hostname != null) {
            return hostname.equalsIgnoreCase(comparedHostName);
        } else {
            return false;
        }
    }

    private boolean isSameMethod(final CVSRoot compared) {
        if (method == null) {
        ...
        } else if (method.equals(compared.getMethod())) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean isSamePort(final CVSRoot compared) {
        if (this.isLocal() == compared.isLocal()) {
            if (this.isLocal()) {
                return true;
            } else if (port == compared.getPort()) {
                return true;
            } else {
                try {
                    final Connection c1 = ConnectionFactory.getConnection(this);
                    final Connection c2 = ConnectionFactory.getConnection(compared);
                    // Test actual ports used by the connections.
                    // This is necessary in case that port in CVSRoot is zero
                    // and the conection is using some default port number.
    ...
    private boolean isSameUser(final CVSRoot compared) {
        final String user = compared.getUserName();
        if (user == getUserName()) {
            return true;
        }
        if (user != null) {
    ...
    /**
     * CVSRoots are equal if their toString representations are equal. This puts
     * some extra pressure on the toString method that should be defined very
     * precisely.
     */
    @Override
    public boolean equals(final Object o) {
        ...
        final CVSRoot compared = (CVSRoot) o;
     ...
     * 
     * @return The connection method or <code>null</code> when no method is
     *         defined.
     */
    ...
    /**
     * setting the method has effects on other components. The method might
     * change - urlFormat - username and password - hostname/port If urlFormat
     * becomes LOCAL_FORMAT then username, password and hostname are set to null
     * and port to 0. If urlFormat becomes SERVER_FORMAT then hostname must not
     * be null.
     */
    ...
    private boolean isLocalMethod(final String method) {
     ...
     * Get the user name.
     * 
     * @return The user name or code>null</code> when the user name is not
     *         defined.
     */
     ...
     * Get the password.
     * 
     * @return The password or <code>null</code> when the password is not
     *         defined.
     */
     ...
     * Get the host name.
     * 
     * @return The host name or <code>null</code> when the host name is not
     *         defined
     */
     ...
     * 
     * @param hostname
     *            The host name or <code>null</code> when the host name is not
     *            defined.
     */
