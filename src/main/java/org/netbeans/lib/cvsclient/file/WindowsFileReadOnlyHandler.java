...
public class WindowsFileReadOnlyHandler implements FileReadOnlyHandler {
     ...
     */
    public void setFileReadOnly(final File file, final boolean readOnly) throws IOException {
        if (file == null) {
            throw new IllegalArgumentException("file must not be null"); // NOI18N
        }

        try {
            final String[] arguments = { "attrib", // NOI18N
                            readOnly ? "+r" : "-r", // NOI18N
                            file.getName() };
            final Process process = Runtime.getRuntime().exec(arguments, null, file.getParentFile());
            process.waitFor();
