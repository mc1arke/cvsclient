 ...
 * security is not an issue. For secure connections, consider using a kserver
 * (Kerberos) or the GSSAPI.
 * 
 * @author Robert Greig
 */
public class PServerConnection extends AbstractConnection {
     ...
     * connection. Note the difference between opening a connection and simply
     * verifying.
     */
     ...
     *             if the cvsRoot does not represent pserver connection type.
     */
    public PServerConnection(final CVSRoot cvsRoot, final SocketFactory factory) {
        if (!CVSRoot.METHOD_PSERVER.equals(cvsRoot.getMethod())) {
            throw new IllegalArgumentException("CVS Root '" + cvsRoot
                            + "' does not represent :pserver: connection type.");
        }
        ...
        final String password = cvsRoot.getPassword();
        if (password != null) {
    ...
    /**
     * Authenticate a connection with the server, using the specified postamble
     * and preamble.
     * 
     * @param preamble
     *            the preamble to use
     ...
     * @return the socket used to make the connection. The socket is guaranteed
     *         to be open if an exception has not been thrown
     */
    private void openConnection(final String preamble, final String postamble) throws AuthenticationException,
                    CommandAbortedException {
        if (hostName == null) {
            final String locMessage = AuthenticationException.getBundleString("AuthenticationException.HostIsNull"); // NOI18N
        ...
        try {
            ...
            final BufferedInputStream bis = new BufferedInputStream(socket.getInputStream(), 32768);
            final LoggedDataInputStream inputStream = new LoggedDataInputStream(bis);
            ...
            if (Thread.interrupted()) {
                reset();
                final String localMsg = CommandException.getLocalMessage("Client.connectionAborted", null); // NOI18N
            ...
            final byte rawResponse[] = inputStream.readBytes(AUTHENTICATION_SUCCEEDED_RESPONSE_RAW.length());
            ...
            if (AUTHENTICATION_FAILED_RESPONSE_RAW.equals(response)) {
                final String localizedMsg = getLocalMessage("AuthenticationException.badPassword", null);
            ...
            final String locMessage = getLocalMessage("AuthenticationException.AuthenticationFailed", // NOI18N
                            new Object[] { response });
            throw new AuthenticationException("AuthenticationFailed", // NOI18N
                            locMessage);
            ...
            reset();
            final String locMessage = getLocalMessage("AuthenticationException.ConnectException", // NOI18N
                            new Object[] { hostName, Integer.toString(port) });
            throw new AuthenticationException("ConnectException", ex, // NOI18N
                            locMessage);
        } catch (final NoRouteToHostException ex) {
            reset();
            final String locMessage = getLocalMessage("AuthenticationException.NoRouteToHostException", // NOI18N
                            new Object[] { hostName });
            throw new AuthenticationException("NoRouteToHostException", ex, // NOI18N
                            locMessage);
        } catch (final IOException ex) {
            reset();
            final String locMessage = getLocalMessage("AuthenticationException.IOException", // NOI18N
                            new Object[] { hostName });
            throw new AuthenticationException("IOException", ex, locMessage); // NOI18N
        }
        /*
         * catch (Throwable t) { reset(); String locMessage =
         * AuthenticationException.getBundleString(
         * "AuthenticationException.Throwable"); //NOI18N throw new
         * AuthenticationException("General error", t, locMessage); //NOI18N }
         */
    }
    ...
    /**
     * Authenticate with the server. Closes the connection immediately. Clients
     * can use this method to ensure that they are capable of authenticating
     * with the server. If no exception is thrown, you can assume that
    ...
    public void verify() throws AuthenticationException {
        ...
        try {
            ...
            final String locMessage = AuthenticationException.getBundleString("AuthenticationException.Throwable"); // NOI18N
     ...
     * server. This Client will call this method before interacting with the
     * server. It is up to implementing classes to ensure that they are
     * configured to talk to the server (e.g. port number etc.).
     * 
    ...
    private String getLocalMessage(final String key, final Object[] arguments) {
