 ...
 * protocol version 1.10. This encoding is trivial and should not be used for
 * security, but rather as a mechanism for avoiding inadvertant compromise.
 * 
 * @author Robert Greig
 */
public class StandardScrambler implements Scrambler {
    ...
    public String scramble(final String text) {
        final StringBuffer buf = new StringBuffer("A"); // NOI18N

        if (text != null) {
            for (int i = 0; i < text.length(); ++i) {
                final char ch = text.charAt(i);
                final byte newCh = (byte) (shifts[ch & 255] & 255);
