...
/**
 * The class abstracts the CVS concept of an <i>entry line</i>. The entry line
 * is textually of the form:
 * <p>
...
public final class Entry {
    ...
    /**
     * Indicates that the file had conflicts and timestamp matches. It likely
     * means unresolved conflict.
     */
    ...
    /**
     * The revision. There are constants defined for no user file, new user file
     * and user file has to be removed.
     */
    private String revision;

    /**
     * The conflict information. There are constants defined for indicating that
     * conflicts occurred and that the timestamp matches the file
     */
     ...
     */
    public Entry(final String entryLine) {
        init(entryLine);
    }
            ...
            if (slashPositions[1] > 0) {
                ...
                if (slashPositions[4] != (entryLine.length() - 1)) {
                    ...
                    if (tagOrDate.startsWith(TAG)) {
                        ...
                        // MK - I didn't notice any time conversions (according
                        // to timezone)
    ...
    public void setConflict(final String conflict) {
        ...
        if (conflictIndex >= 0) {
            ...
            // the +
            final int timeMatchIndex = dateString.indexOf(TIMESTAMP_MATCHES_FILE);
        ...
        try {
            lastModified = getLastModifiedDateFormatter().parse(dateString);
    ...
    /**
     * Get the sticky tag information. May return null if no tag information was
     * present. If so, you should check for date information. Note that tag and
     * date information cannot both be present.
     * 
    ...
    /**
     * Set the sticky tag information. Setting this will remove any date
    ...
    /**
     * Get sticky date information. May return null if no date information is
     * available. If so, you should check for tag informaton. Note that tag and
     * date information cannot both be present.
     * 
    ...
    /**
     * Set the sticky date information. Note that setting this will remove any
     * tag information that is currently set.
     * 
     * @param theDate
     *            the date to use.
     */
    ...
    public boolean isNewUserFile() {
        return (revision != null) && revision.startsWith(NEW_USER_FILE) && (revision.indexOf(".") < 0);
    }
    ...
    /**
     * Create a string representation of the entry line. Create the standard CVS
     * 1.10 entry line format.
     * <p>
    ...
    public String toString() {
        ...
        if (name != null) {
            ...
            if ((tag != null) && (date == null)) {
                ...
                final String dateString = getDateFormatted();
                buf.append(DATE);
