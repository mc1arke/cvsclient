 ...
 * a CVS directory underneath each checked-out directory. How the files are laid
 * out and managed is not specified by the protocol document.
 * <P>
 * Hence it is envisaged that, eventually, a client could add additional files
 * for higher performance or even change the mechanism for storing the
...
public interface AdminHandler extends Serializable {
     ...
     */
    void updateAdminData(String localDirectory, String repositoryPath, Entry entry, GlobalOptions globalOptions)
                    throws IOException;
     ...
     */
    Iterator<Entry> getEntries(File directory) throws IOException;
     ...
     * 
     * @return the value of CVS/Tag file for the specified directory (including
     *         leading "T") null if file doesn't exist
     */
    String getStickyTagForDirectory(File directory);

    /**
     * Tests for existence of the given file. Normally this method delegates to
     * File.exists() but it may also return true for files that exists only
     * virtually (in memory). Is such case the file/directory will not exist on
     * disk but its metadata will be available via getEntries() methods.
     * 
     * @param file
