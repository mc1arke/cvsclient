...
public class export extends AbstractCommandProvider {
    ...
    public Command createCommand(final String[] args, final int index, final GlobalOptions gopt, final String workDir) {
        ...
        if ((command.getExportByDate() == null) && (command.getExportByRevision() == null)) {
            throw new IllegalArgumentException("cvs [export]: "
                            + ResourceBundle.getBundle(export.class.getPackage().getName() + ".Bundle").getString(
                                            "export.Msg_NeedTagOrDate")); // NOI18N)
        }
        final int modulesArgsIndex = go.optIndexGet();
        // test if we have been passed any module arguments
        if (modulesArgsIndex < args.length) {
            final String[] modulesArgs = new String[args.length - modulesArgsIndex];
