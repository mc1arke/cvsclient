...
public class update extends AbstractCommandProvider {
    ...
    public Command createCommand(final String[] args, final int index, final GlobalOptions gopt, String workDir) {
        ...
        while ((ch = go.getopt()) != GetOpt.optEOF) {
            ...
            /*
             * if ((char)ch == 'R') command.setRecursive(true); else if
             * ((char)ch == 'l') command.setRecursive(false); else if ((char)ch
             * == 'd') command.setBuildDirectories(true); else if ((char)ch ==
             * 'P') command.setPruneDirectories(true); else
             */
            if (!ok) {
                usagePrint = true;
            }
        ...
        if (fileArgsIndex < args.length) {
            final File[] fileArgs = new File[args.length - fileArgsIndex];
