...
public class RemoveBuilder implements Builder {
    ...
    public void parseLine(final String line, final boolean isErrorMessage) {
        if (line.indexOf(SCHEDULING) >= 0) {
            final int endingIndex = line.indexOf('\'');
            final String fn = line.substring(line.indexOf(SCHEDULING) + SCHEDULING.length(), endingIndex).trim();
            addFile(fn);
        ...
        if (line.indexOf(STILL_IN_WORKING) >= 0) {
            final int endingIndex = line.indexOf('\'');
            final String fn = line.substring(line.indexOf(STILL_IN_WORKING) + STILL_IN_WORKING.length(), endingIndex)
                            .trim();
            addFile(fn);
            removeInformation.setRemoved(false);
            outputDone();
    ...
    protected File createFile(final String fileName) {
        ...
        if (fileDirectory == null) {
            ...
            // (for directories, the dir name is always sent before the actual
            // files)
            final File locFile = removeCommand.getFileEndingWith(fileName);
            if (locFile == null) {
    ...
    protected void addFile(final String name) {
