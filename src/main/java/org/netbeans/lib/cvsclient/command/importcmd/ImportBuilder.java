...
public class ImportBuilder implements Builder {
    ...
    public ImportBuilder(final EventManager eventManager, final ImportCommand importCommand) {
        ...
        localPath = importCommand.getLocalDirectory();
        module = importCommand.getModule();
    }
    ...
    public void outputDone() {
        ...
        final FileInfoEvent event = new FileInfoEvent(this, fileInfoContainer);
    ...
    public void parseLine(final String line, final boolean isErrorMessage) {
        if ((line.length() > 2) && (line.charAt(1) == ' ')) {
            ...
            if (FILE_INFOS.indexOf(firstChar) >= 0) {
                final String filename = line.substring(2).trim();
    ...
    private void processFile(final String type, String filename) {
        outputDone();
