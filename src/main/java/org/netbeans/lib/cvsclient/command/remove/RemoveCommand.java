...
public class RemoveCommand extends BasicCommand {
     ...
     * would not exist. This is a extension to the standard cvs-behaviour!
     * 
     * @deprecated
     */
    @Deprecated
     ...
     * before. This is a extension to the standard cvs-behaviour!
     */
    ...
    public String getCVSCommand() {
        final StringBuffer toReturn = new StringBuffer("remove "); // NOI18N
    ...
    /**
     * Deletes all files being removed from the working directory. Doesn't
     * delete directories. Attempts a recursive delete
     * 
     * @throws CommandException
     *             - in case the file cannot be deleted.
     */
    private void removeAll(final File[] filesToDel) throws CommandException {
        if (filesToDel == null) {
            return;
        }
        for (final File element : filesToDel) {
            final File file = element;
            if (file.exists() && file.isFile()) {
                if (!file.delete()) {
                    throw new CommandException("Cannot delete file " + file.getAbsolutePath(), // NOI18N
                                    CommandException.getLocalMessage("RemoveCommand.cannotDelete",
                                                    new Object[] { file.getAbsolutePath() })); // NOI18N
                }
            } else {
