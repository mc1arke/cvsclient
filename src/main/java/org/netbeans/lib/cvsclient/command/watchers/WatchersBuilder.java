...
public class WatchersBuilder implements Builder {
    ...
    /**
     * The directory where the command was executed. Used to compute absolute
     * path to the file.
     */
    ...
    public void parseLine(final String line, final boolean isErrorMessage) {
        ...
        final int spaceIndex = trimmedLine.indexOf(' ');

        BugLog.getInstance().assertTrue(spaceIndex > 0, "Wrong line = " + line);

        final File file = new File(localPath, trimmedLine.substring(0, spaceIndex));
        final String watcher = trimmedLine.substring(spaceIndex + 1);
        watchersInfo = new WatchersInformation(file);
