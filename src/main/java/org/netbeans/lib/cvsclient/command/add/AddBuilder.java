...
public class AddBuilder implements Builder {
    ...
    public void outputDone() {
        if (addInformation != null) {
            final FileInfoEvent event = new FileInfoEvent(this, addInformation);
    ...
    public void parseLine(final String line, final boolean isErrorMessage) {
        if (line.endsWith(ADDED)) {
            ...
            final String filename = line.substring(line.indexOf(READDING) + READDING.length(), line.indexOf('('))
                            .trim();
            addFile(filename);
        } else if (line.endsWith(RESURRECTED)) {
            final String filename = line.substring(0, line.length() - RESURRECTED.length());
    ...
    private void addDirectory(final String name) {
        addInformation = new AddInformation();
        addInformation.setType(AddInformation.FILE_ADDED);
        final String dirName = name.replace('\\', '/');
        /*
         * int index = dirName.lastIndexOf('/'); if (index > 0) { dirName =
         * dirName.substring(index + 1, dirName.length()); }
         */
        addInformation.setFile(createFile(dirName));
        outputDone();
    }

    private void addFile(final String name) {
        addInformation = new AddInformation();
    ...
    private void resurrectFile(final String line) {
        final int versionIndex = line.lastIndexOf(RESUR_VERSION);
        final String version = line.substring(versionIndex + RESUR_VERSION.length()).trim();
        final String cutLine = line.substring(0, versionIndex).trim();
        final int fileIndex = cutLine.lastIndexOf(' ');
        final String name = cutLine.substring(fileIndex).trim();
