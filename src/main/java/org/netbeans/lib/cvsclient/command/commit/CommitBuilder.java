...
public class CommitBuilder implements Builder {
    ...
    /**
     * The directory in which the file being processed lives. This is absolute
    ...
    public void parseLine(final String line, final boolean isErrorMessage) {
        ...
        if (line.indexOf(UNKNOWN) >= 0) {
            ...
            final int nameIndex = fileName.lastIndexOf('/');
            if (nameIndex != -1) { // #73181 happens with 1.12 servers:
                                   // /usr/cvsrepo/Java112/nbproject/project.properties,v
            ...
            if (fileDirectory == null) {
                ...
                if (c > 0) {
                    reposPath = reposPath.substring(0, c); // remove the file
                                                           // name
                }
    ...
    private void processUnknownFile(final String line) {
        commitInformation = new CommitInformation();
        commitInformation.setType(CommitInformation.UNKNOWN);
        final int index = line.indexOf('\'');
        final String fileName = line.substring(0, index).trim();
        commitInformation.setFile(createFile(fileName));
        outputDone();
    }

    private void processToAddFile(final String line) {
        commitInformation = new CommitInformation();
    ...
    private void processRevision(String revision) {
        final int index = revision.indexOf(';');
        if (index >= 0) {
    ...
    private File findFile(final String fileName, String reposPath) {
        ...
        final File file = quickFindFile(dir, fileName, reposPath);
        if (file != null) {
            return file;
        }
    ...
    private File findFile(final File dir, final String fileName, final String reposPath) {
            ...
            if (subFiles != null) {
                for (final File subFile : subFiles) {
                    if (subFile.isDirectory()) {
                        file = findFile(subFile, fileName, reposPath);
                        if (file != null) {
                            break;
                        }
                    }
    ...
    private File quickFindFile(File dir, final String fileName, final String reposPath) {
        for (;;) {
            final File deepDir = new File(dir, reposPath);
    ...
    private boolean isWorkForRepository(final File dir, final String reposPath) {
        try {
            String repository = eventManager.getClientServices().getRepositoryForDirectory(dir);
            final String root = eventManager.getClientServices().getRepository();
    ...
    private boolean isUnderLocalPath(File dir) {
        final File localDir = new File(localPath);
        while (dir != null) {
