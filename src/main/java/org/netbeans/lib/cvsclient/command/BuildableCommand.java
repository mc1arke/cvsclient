...
public abstract class BuildableCommand extends Command {
    ...
    public void messageSent(final BinaryMessageEvent e) {
        ...
        if (builder instanceof BinaryBuilder) { // XXX assert it?
            final BinaryBuilder binaryBuilder = (BinaryBuilder) builder;
    ...
    public void messageSent(final MessageEvent e) {
        ...
        if (e instanceof EnhancedMessageEvent) {
            final EnhancedMessageEvent eEvent = (EnhancedMessageEvent) e;
            builder.parseEnhancedMessage(eEvent.getKey(), eEvent.getValue());
            return;
        }

        if (e.isTagged()) {
            final String message = MessageEvent.parseTaggedMessage(taggedLineBuffer, e.getMessage());
            if (message != null) {
            ...
            // #67337 do not interpret piped data using platform default
            // encoding
            // UTF-8 causes problems as raw data (non UTf-8) can contain
            // confusing sequences
            // use safe encoding that does not interpret byte sequences
            if ((builder instanceof PipedFilesBuilder) && (e.isError() == false)) {
                try {
                    final String iso88591 = new String(e.getRawData(), "ISO-8859-1");
    ...
    /**
     * Used for setting user-defined builder. Can be also set null, in that case
     * the builder mechanism is not used at all.
     */
