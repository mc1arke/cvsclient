...
public class StatusBuilder implements Builder {
    ...
    public StatusBuilder(final EventManager eventManager, final StatusCommand statusCommand) {
        ...
        final File[] fileArray = statusCommand.getFiles();
        if (fileArray != null) {
        ...
        localPath = statusCommand.getLocalDirectory();
    ...
    public void parseLine(final String line, final boolean isErrorMessage) {
        if (readingTags) {
            ...
            if (bracket > 0) {
                // it's another tag..
                final String tag = line.substring(0, bracket).trim();
                final String rev = line.substring(bracket + 2, line.length() - 1);
        ...
        if (line.startsWith(UNKNOWN_FILE) && beginning) {
            final File file = new File(localPath, line.substring(UNKNOWN_FILE.length()));
            statusInformation = new StatusInformation();
        ...
        if (line.startsWith(UNKNOWN)) {
            ...
            /*
             * if (statusInformation.getRepositoryRevision().startsWith(
             * NOT_IN_REPOSITORY)) { outputDone(); }
             */
    ...
    private File createFile(final String fileName) {
        ...
        if (relativeDirectory != null) {
            ...
            for (int i = 0; i < fileArray.length; i++) {
                final File currentFile = fileArray[i];
    ...
    private void processFileAndStatusLine(final String line) {
        ...
        final String status = new String(line.substring(statusIndex + 8).trim());
    ...
    private void processWorkRev(final String line) {
        if (!assertNotNull()) {
            return;
        }
    ...
    private void processTag(final String line) {
        if (!assertNotNull()) {
            return;
        }
    ...
    private void processOptions(final String line) {
        if (!assertNotNull()) {
            return;
        }
