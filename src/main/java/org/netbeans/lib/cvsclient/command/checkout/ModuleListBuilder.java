...
public class ModuleListBuilder implements Builder {
    ...
    public ModuleListBuilder(final EventManager eventMan, final CheckoutCommand comm) {
    ...
    public void parseLine(String line, final boolean isErrorMessage) {
        line = line.replace('\t', ' ');
    ...
    protected void processModule(final String line, final boolean firstLine) {
        final StringTokenizer tok = new StringTokenizer(line, " ", false); // NOI18N
        if (firstLine) {
            outputDone();
            moduleInformation = new ModuleListInformation();
            final String modName = tok.nextToken();
            moduleInformation.setModuleName(modName);
            if (checkoutCommand.isShowModulesWithStatus()) {
                final String stat = tok.nextToken();
