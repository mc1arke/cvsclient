...
/**
 * Describes "cvs watchers" commands' parsed information for a file. The fields
 * in instances of this object are populated by response handlers.
 * 
 * @author Milos Kleint
 */
public class WatchersInformation extends FileInfoContainer {
    ...
    /**
     * List of users (Watchers instances) that are listening on events for this
     * file.
     */
     ...
     * 
     * @param watchingInfo
     *            a String that's first word is a user name and the rest are
     *            watching types.
     */
    void addWatcher(final String watchingInfo) {
        String temp = watchingInfo.trim();
        temp = temp.replace('\t', ' ');
        final int spaceIndex = temp.indexOf(' ');
        if (spaceIndex < 0) {
            // BUGLOG assert.
        } else {
            final String user = temp.substring(0, spaceIndex);
            final String watches = temp.substring(spaceIndex + 1);
            userList.add(new WatchersInformation.Watcher(user, watches));
        }
    }

    /**
     * Returns the Iterator with WatchersInformation.Watcher instances. Never
     * returns null.
     */
    ...
    public static class Watcher {
        ...
        /**
         * Package private constuctor that creates a new instance of the
         * Watcher. To Be called from outerclass only.
         */
        Watcher(final String userName, final String watches) {
