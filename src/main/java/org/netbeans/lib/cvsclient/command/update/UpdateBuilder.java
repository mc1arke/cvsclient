...
public class UpdateBuilder implements Builder {
    ...
    // cotacao/src/client/net/riobranco/common/client/gui/BaseDialogThinlet.java
    // already contains the differences between 1.17 and 1.18
    ...
    /**
     * Holds 'G' or 'C' if the current file was merged or conflicted,
     * respectively.
     */
    ...
    public void parseLine(final String line, final boolean isErrorMessage) {
        ...
        if (line.indexOf(UNKNOWN) >= 0) {
        ...
        } else if (line.indexOf(EXAM_DIR) >= 0) { // never comes with :local;
                                                  // connection method
            return;
            ...
            if (line.indexOf(PERTINENT) > 0) {
                final String filename = line.substring(line.indexOf(WARNING) + WARNING.length(),
            ...
            if (line.indexOf(REMOVAL_12) > 0) {
                final String filename = line.substring(
                                line.indexOf(SERVER_SCHEDULING_12) + SERVER_SCHEDULING_12.length(),
                                line.indexOf(REMOVAL_12)).trim();
                processNotPertinent(filename);
            }
        } else if (line.indexOf(SERVER_SCHEDULING) >= 0) {
            if (line.indexOf(REMOVAL) > 0) {
                final String filename = line.substring(line.indexOf(SERVER_SCHEDULING) + SERVER_SCHEDULING.length(),
                                line.indexOf(REMOVAL)).trim();
                processNotPertinent(filename);
            }
        } else if (line.indexOf(MERGE_SAME) >= 0) { // not covered by
                                                    // parseEnhancedMessage
            ensureExistingFileInfoContainer();
            fileInfoContainer.setType(DefaultFileInfoContainer.MERGED_FILE);
            final String path = line.substring(0, line.indexOf(MERGE_SAME));
            fileInfoContainer.setFile(createFile(path));
            outputDone();
        } else if (line.startsWith(MERGED)) { // not covered by
                                              // parseEnhancedMessage
            outputDone();
            ...
            // otherwise
            if (line.length() > 2) {
                final String firstChar = line.substring(0, 2);
    ...
    private void processUnknownFile(final String line, final int index) {
        outputDone();
        fileInfoContainer = new DefaultFileInfoContainer();
        fileInfoContainer.setType("?"); // NOI18N
        final String fileName = (line.substring(index)).trim();
    ...
    private void processFile(String line) {
        ...
        if (fileInfoContainer != null) {
            // sometimes (when locally modified.. the merged response is
            // followed by mesage M <file> or C <file>..
            // check the file.. if equals.. it's the same one.. don't send
            // again.. the prior type has preference
            if (fileInfoContainer.getFile() == null) {
                // is null in case the global switch -n is used - then no
                // Enhanced message is sent, and no
                // file is assigned the merged file..
                fileInfoContainer.setFile(file);
            }
            if (file.equals(fileInfoContainer.getFile())) {
                // if the previous information does not say anything, prefer
                // newer one
    ...
    private void processNotPertinent(final String fileName) {
        outputDone();
        final File fileToDelete = createFile(fileName);

        ensureExistingFileInfoContainer();

        // HACK - will create a non-cvs status in order to be able to have
        // consistent info format
